...
void carregarSecoesMateriais(Secao **head, Material **materiais) {
    ...
    while (fgets(buffer, sizeof(buffer), arquivo) != NULL) {
        if (sscanf(buffer, "Seção: %[^\n]", nomeSecao) == 1) {
            ...
            fgets(buffer, sizeof(buffer), arquivo); // Ler a linha seguinte contendo a localização
            ...
            fgets(buffer, sizeof(buffer), arquivo); // Ler a linha seguinte contendo o tipo do material
            sscanf(buffer, "    Tipo: %[^\n]", tipoMaterial);
            strcpy(novoMaterial->tipo, tipoMaterial);

            fgets(buffer, sizeof(buffer), arquivo); // Ler a linha seguinte contendo o preço e a quantidade
